    ...
    std::vector<std::vector<char>>* pFrames)
{
    ...
    if (formatDesc == nullptr) {
        PPX_LOG_ERROR("Failed to fetch information for texture format " <<
            format);
        return ppx::ERROR_FAILED;
    }
    ...
    if (size % frameSizeInBytes != 0) {
        PPX_LOG_WARN("There may be partial frames in the raw video file at " <<
            path << "; loading as much as possible.");
    }
    ...
    grfx::ResourceState stateAfter)
{
    ...
    PPX_ASSERT_MSG(formatDesc != nullptr, "Cannot find format description!");
    std::optional<grfx::FormatPlaneDesc> planeDesc =
        grfx::GetFormatPlaneDescription(pImage->GetFormat());
    PPX_ASSERT_MSG(planeDesc.has_value(), "This is only used by planar formats for now!");

    // Copy info
    std::vector<grfx::BufferToImageCopyInfo> copyInfos;
    uint32_t footprintOffset = 0;  // We'll keep a running tally, per plane.
    for (size_t i = 0; i < planeDesc->planes.size(); ++i) {
        grfx::FormatPlaneDesc::Plane& plane = planeDesc->planes[i];
        uint32_t copyWidth = GetPlaneWidthForCopy(plane, formatDesc->chromaSubsampling, pImage->GetWidth());
        uint32_t copyHeight = GetPlaneHeightForCopy(plane, formatDesc->chromaSubsampling, pImage->GetHeight());
        uint32_t planeSize = GetPlaneSizeInBytes(plane, formatDesc->chromaSubsampling, pImage->GetWidth(), pImage->GetHeight());

        grfx::BufferToImageCopyInfo copyInfo = {};
        copyInfo.srcBuffer.imageWidth        = 0;
        copyInfo.srcBuffer.imageHeight       = 0;
        copyInfo.srcBuffer.imageRowStride    = 0;
        copyInfo.srcBuffer.footprintOffset   = footprintOffset;
        ...
        copyInfo.dstImage.y                  = 0;
        copyInfo.dstImage.z                  = 0;
        copyInfo.dstImage.width              = copyWidth;
        copyInfo.dstImage.height             = copyHeight;
        copyInfo.dstImage.depth              = 1;
        copyInfos.push_back(copyInfo);

        // Add plane's size to footprint offset for the next plane.
        footprintOffset += planeSize;
    }
