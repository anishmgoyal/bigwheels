...
// Definitions for image planes - component types and bit sizes. We'll define
// and undefine a helper macro to make the plane descriptions more readable.
    ...
    grfx::Format format)
{
    ...
    if (description == nullptr || !description->isPlanar) {
        PPX_LOG_WARN("Attempted to get planes for format " << format <<
            ", which is non-planar");
    } else {
        PPX_LOG_ERROR("Could not find planes for planar format " << format <<
            "; returning null.");
    }
